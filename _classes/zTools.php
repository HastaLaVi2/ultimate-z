...
class zTools {
    ...
    public function zToolsCreate($data) {
        ...
        if(!empty($data)) {
            ...
            $query = $db->execute("INSERT INTO `z` (name, create_date, cookie_key, status, error, hidden_key, e_commerce) VALUES ('$name', CURRENT_TIMESTAMP, '$cookie_key', 'enabled', 'enabled', '$hidden_key', '')");
            if($query) {} else {
