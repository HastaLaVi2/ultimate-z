...
class zPageStarter extends zPage {
    ...
    public function _zStarter(array $data = NULL, $zSmarty = NULL) {
        ...
        foreach ($zTools->zToolsGetAllLangs() as $l) {
            $editPage[$l->id] = new zCategory($_GET["id_category"], $l->id);
        }
        $zSmarty->assign("editPage", $editPage);

        if(!empty($data)) {
            if (isset($data["id_lang"]) && isset($data["id_category"]) && isset($data["category_name"])) {
                try {
                    $data = $zCategoryTools->zCategoryUpdate($data);
                    if ($data) {
